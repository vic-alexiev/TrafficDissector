...
namespace SharpPcap.Packets
{
    ...
    public class TCPPacket : IPPacket
    {
        ...
        // NB! Added by Victor Alexiev
        public virtual int UrgentPointer
        {
            get
            {
                if (!_urgentPointerSet)
        ...
        public override System.String ToColoredString(bool colored)
        {
            ...
            if (Urg)
                buffer.Append(" urg[0x" + Convert.ToString(getUrgentPointer(), 16) + "]");
            if (Ack)
                buffer.Append(" ack[0x" + Convert.ToString(AcknowledgmentNumber, 16) + "]");
            if (Psh)
                buffer.Append(" psh");
            if (Rst)
                buffer.Append(" rst");
            if (Syn)
                buffer.Append(" syn[0x" + Convert.ToString(SequenceNumber, 16) + "," +
                              SequenceNumber + "]");
            if (Fin)
                buffer.Append(" fin");
